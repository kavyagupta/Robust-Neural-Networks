...
def input_init(x):
    return tf.linalg.matmul(x, tf.eye(513, 1539))

def get_output(x):
    return  tf.linalg.matmul(x, tf.eye(1539, 513))
    ...
    def sparse_matrix(self, w):
        for i in range(w.shape[0]):
            for j in range(w.shape[1]):
                if(np.absolute((w.shape[1] - 1) * (i - 1) - (w.shape[0] - 1)*(j - 1)) >= ((np.maximum(w.shape[0], w.shape[1]) / 2) - 20) * (np.maximum(w.shape[0], w.shape[1]) - 1)):
                    w[i][j] = 0
        return w
...
if __name__=='__main__' :
    ...
    path_save = '../Save/'
    name = 'data_0.5window'
    model_name = '../Models/mimo_test3_ch.h5'
   # save_model = '../Models/mimo_FCN_L1_3.h5'
    n_factor = 50
...
#    x_train, x_val, x_test, y_train, y_val, y_test, S1, sr = make_input(path, name, N, path_save )
  ...
  #  w = get_weights(prev_model)
    ...
    Net.summary()
   # Net = load_model(save_model, custom_objects={'Norm_Constraint': Norm_Constraint, 'SNR_compute': SNR_compute, 'input_init': input_init, 'get_output': get_output, 'tf': tf})
    Net.fit(x_train, y_train, epochs=2, batch_size=1024, shuffle=True, validation_data=(x_val, y_val), callbacks=[checkpoint, cnstr_weight])
    check_norms(Net)
    save_weights(Net)
