...
def make_input(path, name, N, path_save):
    ...
    X_train = np.empty((0, 1026))
    Y_train = np.empty((0, 1026))

    X_val = np.empty((0, 1026))
    Y_val = np.empty((0, 1026))

    X_test = np.empty((0, 1026))
    Y_test = np.empty((0, 1026))
    ...
    for folder in folders:
        ...
        for file in files:
            if file.split('.')[-1] == 'wav':
                ...
                music_1 = audio[0,...]#/np.max(np.abs(audio[0,...]))
                music_2 = audio[1,...]#/np.max(np.abs(audio[1,...]))

                #print(len(music_1))

                if folder == 'Test':
                    #wavfile.write('../Tests/test_clean.wav', Fs, audio)
                    librosa.output.write_wav('../Tests/test_clean.wav', audio, sr)
                    print(audio.shape)
                ...
                sample_1 = np.asanyarray(music_1)
                sample_2 = np.asanyarray(music_2)

                signal_avg_watts = np.mean(audio ** 2)
          ...
          #      print('noise_avg_watts ', noise_avg_watts)
                ...
                noise_volts = k * np.random.normal(mean_noise, np.sqrt(noise_avg_watts), len(sample_1))
                noisy_signal_1 = sample_1 + noise_volts
                noisy_signal_2 = sample_2 + noise_volts

                #pure_audio = audio
                #audio = noisy_signal

                S1=librosa.stft(np.asfortranarray(sample_1), n_fft=1024, hop_length=512)
                #S1 = S1/np.max(np.abs(S1))
                S2 =librosa.stft(np.asfortranarray(sample_2), n_fft=1024, hop_length=512)
                #S2 = S1/np.max(np.abs(S2))

                X1=librosa.stft(noisy_signal_1, n_fft=1024, hop_length=512)
                #X1 = S1/np.max(np.abs(X1))
                X2=librosa.stft(noisy_signal_2, n_fft=1024, hop_length=512)
                #X2 = S1/np.max(np.abs(X2))
                
                
                #print('hereeeee',X.shape)
                abs_S = np.append(np.abs(S1.T),np.abs(S2.T), axis = 1)
                print(abs_S.shape)
                abs_X = np.append(np.abs(X1.T),np.abs(X2.T), axis = 1)
                print(abs_S.shape)
                ...
                if folder == 'Test':
                  ...
                  #librosa.output.write_wav('../Tests/test_load.wav', noisy_signal, Fs)
   ...
   # print(np.allclose(x_test,y))
    ...
    return X_train, X_val, X_test, X_train, Y_val, Y_test, X, F_S1
